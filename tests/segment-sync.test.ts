...
describe('normalizeMentionRoleId（区分ロール指定の検証・正規化）', () => {
  const GUILD = '100000000000000001';
  it('空・null・空白のみは手動管理(null)', () => {
    expect(normalizeMentionRoleId(null, GUILD)).toBe(null);
    expect(normalizeMentionRoleId(undefined, GUILD)).toBe(null);
    expect(normalizeMentionRoleId('', GUILD)).toBe(null);
    expect(normalizeMentionRoleId('  ', GUILD)).toBe(null);
  });
  it("'@everyone' センチネルはそのまま", () => {
    expect(normalizeMentionRoleId('@everyone', GUILD)).toBe('@everyone');
  });
  it('ロールIDはそのまま（前後空白は除去）', () => {
    expect(normalizeMentionRoleId('400000000000000001', GUILD)).toBe('400000000000000001');
    expect(normalizeMentionRoleId(' 400000000000000001 ', GUILD)).toBe('400000000000000001');
  });
  it('@everyone の実ID（= guild_id）は @everyone に正規化', () => {
    expect(normalizeMentionRoleId(GUILD, GUILD)).toBe('@everyone');
  });
  it('ロール名・<@&id> 形式・非文字列は不正(undefined)', () => {
    expect(normalizeMentionRoleId('キャスト', GUILD)).toBe(undefined);
    expect(normalizeMentionRoleId('<@&400000000000000001>', GUILD)).toBe(undefined);
    expect(normalizeMentionRoleId(123, GUILD)).toBe(undefined);
  });
});
...
describe('syncSegmentFromRole（完全同期・MOCK・ADR 0009）', () => {
  ...
  it('ロールを失った人は除外し、休止状態は保持される', async () => {
    ...
    await addSegmentMember(db(), seg,'3003'); // 非ロール者(miki)を手動投入
    const r = await syncSegmentFromRole(env, seg, { allowEmpty: true });
  ...
  it('0人かつ既存ありで allowEmpty=false は無人化を防いでスキップ（既存維持）', async () => {
    ...
    await addSegmentMember(db(), seg,'3001');
    const r = await syncSegmentFromRole(env, seg, { allowEmpty: false });
    expect(r.ok).toBe(false);
    expect((await listSegmentMembers(db(), seg.id)).length).toBe(1); // 消えていない
  });

  it('保有者0人の同期は成功扱いだが警告メッセージを含む（ロール削除・指定誤りの検知）', async () => {
    const seg = await createSegment(db(), { guild_id: '1001', name: '空ロール2', mention_role_id: '9999' });
    const r = await syncSegmentFromRole(env, seg, { allowEmpty: true });
    expect(r.ok).toBe(true);
